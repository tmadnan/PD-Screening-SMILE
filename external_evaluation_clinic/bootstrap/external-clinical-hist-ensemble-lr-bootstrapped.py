...
def bootstrap_evaluation(model, X_test, y_test, n_iterations=1000, random_state=42, output_file='bootstrap_results.csv'):
    np.random.seed(random_state)

    # Identify PD and Non-PD samples
    ...
    metrics = {
        'Accuracy': [], 'AUROC': [], 'Sensitivity': [], 'Specificity': [],
        'PPV': [], 'NPV': [], 'F1-Score': []
    }

    all_iterations_df = []

    for _ in tqdm(range(n_iterations), desc="Bootstrap Resampling"):
        sampled_pos = np.random.choice(pos_indices, n_pos, replace=True)
        sampled_neg = np.random.choice(neg_indices, n_neg, replace=True)
        sampled_indices = np.concatenate([sampled_pos, sampled_neg])

        X_sample = X_test[sampled_indices]
        y_sample = y_test[sampled_indices]

        y_pred = model.predict(X_sample)
        y_score = model.predict_proba(X_sample)[:, 1]

        # Confusion matrix to derive Specificity and NPV
        cm = confusion_matrix(y_sample, y_pred)
        tn, fp, fn, tp = cm.ravel()

        # Calculate performance metrics
        acc = accuracy_score(y_sample, y_pred)
        auc = roc_auc_score(y_sample, y_score)
        ...
        metrics['AUROC'].append(round(auc*100, 1))
        metrics['Sensitivity'].append(round(sensitivity*100, 1))
        metrics['Specificity'].append(round(specificity*100, 1))
        metrics['PPV'].append(round(ppv*100, 1))
        metrics['NPV'].append(round(npv*100, 1))
        metrics['F1-Score'].append(round(f1*100, 1))

        all_iterations_df.append([
            acc, auc, sensitivity, specificity, ppv, npv, f1
        ])

    # Create DataFrame for all iterations and save
    bootstrap_df = pd.DataFrame(all_iterations_df, columns=metrics.keys())
    ...
    for metric, values in metrics.items():
        ...
        lower_ci, upper_ci = np.percentile(values, [2.5, 97.5])
        ci = (upper_ci - lower_ci) / 4
        results_summary[metric] = f"{mean_val:.1f} ± ({ci:.1f})"
        # lower_ci, upper_ci = np.percentile(values, [2.5, 97.5])
        # results_summary[metric] = f"{mean_val:.1f} ± ({lower_ci:.1f}, {upper_ci:.1f})"

    return results_summary, bootstrap_df
...
def ensemble_mode_external_test_set(arg, cfgs, df, df_test, model_objects, 
    ...
    # # store X_test and y_test in a pickle file
    # with open('external-clinical-X_test.pkl', 'wb') as f:
    #     pickle.dump(X_test, f)
    # with open('external-clinical-y_test.pkl', 'wb') as f:
    #     pickle.dump(y_test, f)
        
    # read X_test and y_test from the pickle file
    with open('external-clinical-X_test.pkl', 'rb') as f:
        X_test = pickle.load(f)
    with open('external-clinical-y_test.pkl', 'rb') as f:
        y_test = pickle.load(f)
    
    # with open('external-clinical-model.pkl', 'wb') as f:
    #     pickle.dump(clf, f)
        
    with open('external-clinical-model.pkl', 'rb') as f:
        clf = pickle.load(f)
    ...
    print("Model Trained")
    print("Evaluating Model using Bootstrapping")
    
    results_summary, bootstrap_df = bootstrap_evaluation(clf, X_test, y_test, n_iterations=1000)
    
    # print results summary as json
...
def run_ensemble_models(df, cfg, df_test):
    ...
    second_line = 'from all three expressions'
    
    if cfg.test_set == 'SuperPD':
        second_line = 'Clinic Cohort'
    elif cfg.test_set == 'BanglaPD':
        second_line = 'from Home-BD'
...
def main():
    ...
    parser.add_argument('--name', type=str, default='All Clinic', help='Name of the ensemble method')
    ...
    parser.add_argument('--test_set', type=str, default='SuperPD', help='Path to the test set')
